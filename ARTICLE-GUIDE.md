...
# Article Guide — toilet.portabel.id
...
Source route evidence: sitemap XML, 3191 routes discovered during packet generation
Reader community name: `Portabel.id`
...
- Address the reader warmly as `Sobat Portabel.id`, `Kawan Portabel.id`, or `Teman Portabel.id`. Use the exact opening salutation assigned inside the article file.
